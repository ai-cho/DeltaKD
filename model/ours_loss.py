...
def ours_loss(student_model, student_features, teacher_features, dist_type, top_k=3):
    ...
    if dist_type == 'test':  
        ...
        for i in range(10):
            ...
            cost_mat = F.cosine_similarity(sf_1[:, 1:, :], sf_2[:, 1:, :], dim=-1) # B, 196
            student_dist = torch.mean(cost_mat, dim=-1)
            student_dist = torch.mean(student_dist, dim=0)
            ...
            cost_mat = F.cosine_similarity(tf_1[:,2:,:], tf_2[:,2:,:], dim=-1) # B, 196
            teacher_dist = torch.mean(cost_mat, dim=-1)
            teacher_dist = torch.mean(teacher_dist, dim=0)

            # cost_mat = torch.cdist(tf_1.reshape(B, -1), tf_2.reshape(B, -1), p=2)
            # dist_t = cost_mat.diag().sum()/B
            # print(f'studnet_dist.shape: {student_dist.shape}')
            # print(f'teacher_dist.shape: {teacher_dist.shape}')
            layer_dist_s.append(student_dist)
            layer_dist_t.append(teacher_dist)
            layer_dist.append(torch.abs(student_dist - teacher_dist))
            # w1 -= 0.1
        ...
        if student_model.align2 is not None:
            ...
            xc = block_0_s
        # print(layer_dist)
        dist, top_k_idx = torch.topk(torch.tensor(layer_dist), top_k)
        loss_mse = nn.MSELoss(reduction='mean')
        loss_mimick = loss_mse(xc, block_0_t)
