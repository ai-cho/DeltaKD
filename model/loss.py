...
from model.ours_loss import ours_loss
...
class DistillationLoss(torch.nn.Module):
    ...
    def forward(self, inputs, outputs, student_features, labels):
        ...
        if not isinstance(outputs, torch.Tensor):
            ...
            outputs, outputs_kd = outputs # 둘 다 (B, 100), outputs_kd: logits
        ...
        with torch.no_grad():
            ...
            teacher_logits, teacher_features = self.teacher_model(inputs) # teacher_logits (B, 100)
        ...
        if self.distillation_type == 'soft':
            ...
            print(f'teacher_logits: {teacher_logits.argmax(dim=1)[0]}')
        ...
        elif self.distillation_type == 'ours':
            student_model = self.student_model.module if isinstance(self.student_model, torch.nn.parallel.DistributedDataParallel) else self.student_model

            distillation_loss = ours_loss(student_model, student_features, teacher_features, 'test', top_k=3)
